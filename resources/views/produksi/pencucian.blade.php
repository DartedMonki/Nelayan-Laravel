...
@section('content')
    @include('listkelompok.partials.header', ['title' => __('Pencucian')])   
                        ...
                        <form method="post" action="{{ route('produksi.update', $produksi->id) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                ...
                                <div class="form-group{{ $errors->has('panjang_ikan') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-panjang-ikan">{{ __('Panjang Ikan Setelah Cuci (cm)') }}</label>
                                ...
                                <div class="form-group{{ $errors->has('jumlah_ikan') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-jumlah-ikan">{{ __('Jumlah Ikan Setelah Cuci') }}</label>
                                ...
                                <div class="form-group{{ $errors->has('tanggal_cuci') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-tgl-tebar">{{ __('Tanggal Cuci') }}</label>
                                    <input type="date" name="tanggal_cuci" id="input-tgl-tebar" class="form-control form-control-alternative{{ $errors->has('tanggal_cuci') ? ' is-invalid' : '' }}" placeholder="{{ __('Tanggal Cuci') }}" value="{{ old('tanggal_cuci', $produksi->tanggal_cuci) }}" required>

                                    @if ($errors->has('tanggal_cuci'))
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $errors->first('tanggal_cuci') }}</strong>
                                        </span>
                                    @endif
                                </div>

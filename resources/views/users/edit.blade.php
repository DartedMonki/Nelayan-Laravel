@extends('layouts.app', ['page' => __('Manajemen User'), 'pageSlug' => 'users'])
                        ...
                        <form method="post" action="{{ route('user.update', $user) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="name" id="input-name" class="form-control form-control-alternative{{ $errors->has('name') ? ' is-invalid' : '' }}" placeholder="{{ __('Nama') }}" value="{{ old('name', $user->name) }}" required autofocus>
                                ...
                                <div class="form-group{{ $errors->has('status') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="status" id="input-status" class="form-control form-control-alternative{{ $errors->has('status') ? ' is-invalid' : '' }}" placeholder="{{ __('Status') }}" value="{{ old('status', $user->status) }}" required autofocus>
